...
public class NoriAnalyzerProvider extends AbstractIndexAnalyzerProvider<KoreanAnalyzer> {
    ...
    public NoriAnalyzerProvider(IndexSettings indexSettings, Environment env, String name, Settings settings) {
        ...
        final boolean searchMode = NoriTokenizerFactory.getSearchMode(settings);
        final List<String> karrotOption = NoriTokenizerFactory.getKarrotOption(settings);
        ...
        analyzer = new KoreanAnalyzer(userDictionary, mode, stopTags, false, true, karrotOption);
    }
